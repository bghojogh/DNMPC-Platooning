...
cutout_cars_in_platoon_so_far = zeros(Num_veh,1);
 ...
 Xend_cutin = zeros(Num_step,Num_veh_cutin); Vend_cutin = zeros(Num_step,Num_veh_cutin);
...
for i = 2:Num_step - Np
    ...
    if (cutin_OnAndOff(cut_in_index) == 1) && (i >= (t_cutin(cut_in_index) * (1 / Tim_step)))
        ...
        if p_cutin(cut_in_index) == 1
            Pd = x0(i-1:i+Np-1) - d;  Vd = v0(i-1:i+Np-1);
        else
